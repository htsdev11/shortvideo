...
# Paste a NEW complete Request Cookie value between the triple quotes.
#
# Do not reuse a cookie that has been posted in chat, logs, screenshots,
# source control, or any other public/shared location.
PINTEREST_COOKIE = """
...
def get_pinterest_cookie(
    ...
    """
    Return an explicitly supplied cookie or the cookie stored
    directly in this functions.py file.
    """
    ...
    if (
        ...
        raise PinterestScraperError(
            "PINTEREST_COOKIE is not configured in functions.py."
        )
...
def save_pinterest_pin(
    ...
    variant = select_best_video_variant(
    ...
    if not variant:
        raise PinterestScraperError(
            f"No usable video variant "
            f"found for Pin {pin_id}."
        )
    ...
    title = (
        ...
        or pin_data.get("seo_title")
        or f"Pinterest video {pin_id}"
    )

    description = (
        pin_data.get("description")
        or pin_data.get("seo_description")
        or ""
    )
...
def refresh_pinterest_video_link(
    ...
    try:
        ...
        if not fresh_variant:
            raise PinterestScraperError(
                f"Pinterest returned no working "
                f"video URL for Pin {pin_id}."
            )
